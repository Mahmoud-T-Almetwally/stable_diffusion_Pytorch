...
def generate(prompt: str,
    ...
    with torch.no_grad():
        ...
        if not (0 < strength <= 1):
            raise ValueError('Strength parameter must be between 0 & 1')
        ...
        if sampler_name == 'ddpm':
            ...
            raise ValueError(f'Unknown smapler, {sampler_name}')
        ...
        if input_image:
            ...
            input_image_tensor = input_image.resize((WIDTH, HIEGHT))
...
def get_time_embedding(timestep):
    freqs = torch.pow(10_000, -torch.arange(start=0, end=160) / 160)
